...
const EventBlock = ({
  ...
  const bgSelector = !!event?.task
    ? JSON.parse(event?.task)?.activity?.status
    : 'transparent';
  const isDone = event?.task
    ? JSON.parse(event.task)?.activity?.status == 'Done' ||
      JSON.parse(event.task)?.activity?.status == 'No'
    : false;

  const isMaybe = React.useMemo(() => {
    if (event.task && JSON.parse(event.task)?.activity?.status == 'Maybe')
      return true;
    return false;
  }, [event]);
        ...
        style={[
          StyleSheet.absoluteFill,
          {
            backgroundColor:
              isDone || isMaybe
                ? 'transparent'
                : bgSelector == undefined
                ? '#F4F4F4'
                : event.color,
            borderWidth: isDone || isMaybe ? 1 : 0,
            borderStyle: isDone || isMaybe ? 'dashed' : 'solid',
